...
impl Db {
    ...
    pub fn get_or_create_open_segment(&self, agent_id: &str, namespace: &str) -> Result<i64> {
        ...
        if let Some((id, size)) = maybe {
            if size < DEFAULT_SEGMENT_CAPACITY {
                return Ok(id);
            }
            // close it
            let now = now_ms();
    ...
    pub fn tombstone_delete(&self, agent_id: &str, id: &str) -> Result<bool> {
        let seg_id: Option<i64> = self.conn.query_row(
            "SELECT segment_id FROM memories WHERE id=?1 AND agent_id=?2 AND is_deleted=0",
            params![id, agent_id], |r| r.get(0),
        ).ok();
